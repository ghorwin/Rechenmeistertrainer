    ...
    const QRectF &rect ) const
{
    ...
    switch ( axisId )
    {
        ...
        case QwtPlot::yRight:
        {
            // move scale left by one to avoid gap between frame and scale
            x = rect.left() + baseDist - 1;
            y = rect.y() + startDist;
            w = rect.height() - startDist - endDist;
        ...
        {
            x = rect.left() + startDist;
            // move scale up by one to avoid gap between frame and scale
            y = rect.top() + baseDist - 1;
            w = rect.width() - startDist - endDist;
    ...
    const QwtScaleMap *map ) const
{
    ...
    // move right boundary by width of backbone of right scale
